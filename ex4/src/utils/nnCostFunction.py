...
#
#     J = J + ( (Lambda/(2.0*m))*(sumOfTheta1+sumOfTheta2) )
#
#     #%% BACKPROPAGATION
#
#     bigDelta1 = 0
...
#     Theta1_grad += (float(Lambda)/m)*Theta1
#     Theta2_grad += (float(Lambda)/m)*Theta2
...
def nnCostFunction(nn_params, input_layer_size, first_hidden_layer_size, second_hidden_layer_size, num_labels, X, y, Lambda):
    ...
    v = np.log(htheta) * y
    v2 = np.log(1 - htheta + eps) * (1 - y)
    v3 = np.sum(Theta1[:, 1:] ** 2)
    v4 = np.sum(Theta2[:, 1:] ** 2)
    v5 = np.sum(Theta3[:, 1:] ** 2)  # regularization term for Theta3

    J = -1 / m * np.sum((np.sum(np.log(htheta) * y) + np.sum(np.log(1 - htheta + eps) * (1 - y))
                         - Lambda / 2 * (np.sum(Theta1[:, 1:] ** 2) + np.sum(Theta2[:, 1:] ** 2) + np.sum(
                Theta3[:, 1:] ** 2))))
    ...
    Theta3_grad[:, 1:] += Lambda / m * Theta3[:, 1:]
